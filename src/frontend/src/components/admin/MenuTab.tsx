...
import { Loader2, Plus, Tag, Trash2, UtensilsCrossed } from "lucide-react";
...
export default function MenuTab() {
  // Category form state
  const [catOutletId, setCatOutletId] = useState("");
  const [catName, setCatName] = useState("");
  const [filterCatOutletId, setFilterCatOutletId] = useState<string | null>(
    null,
  );
  ...
  const { data: allCategories = [], isLoading: catsLoading } =
    useMenuCategories(null);
  const { data: menuItems = [], isLoading: itemsLoading } =
    useMenuItems(filterOutletId);
  ...
  const filteredCategories = filterCatOutletId
  ...
  return (
    <div data-ocid="menu.section" className="space-y-6">
      {/* ---- CATEGORIES SECTION ---- */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Select
                ...
                disabled={!newOutletId}
              >
                <SelectTrigger data-ocid="menu.category.select">
                  <SelectValue
                    placeholder={
                      newOutletId
                        ? itemFormCategories.length === 0
                          ? "No categories yet"
                          : "Select category"
                        : "Select outlet first"
                    }
